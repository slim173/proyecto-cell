...
public static class DbMigrator
{
    ...
    private static async Task SeedEmpresaConfigAsync(System.Data.IDbConnection conn, ILogger? logger)
    {
        ...
        var datos = new (string clave, string valor, string desc)[]
        {
            ...
            ("wa_msg_entrada",    "¡Hola {nombre}! Hemos recibido tu {dispositivo} en DOCTOR MOVIL. Orden #{orden}. Te avisamos en cuanto esté listo. ¡Gracias!\n🔗 Estado en tiempo real: {link_portal}", "WA — entrada registrada"),
            ("wa_msg_listo",       "¡Hola {nombre}! Tu {dispositivo} ya está listo para recoger. Orden #{orden} · {precio}€. Pasa cuando quieras por DOCTOR MOVIL.\n🔗 Ver estado: {link_portal}", "WA — listo para recoger"),
            ("wa_msg_recordatorio","Hola {nombre}, te recordamos que tienes el {dispositivo} (Orden #{orden}) pendiente de recoger en DOCTOR MOVIL. Importe: {precio}€. ¿Cuándo puedes pasarte?\n🔗 Ver estado: {link_portal}", "WA — recordatorio"),
